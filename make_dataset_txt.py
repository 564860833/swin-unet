# 解析命令行参数，灵活配置数据处理流程。
# 加载.npz格式的3D医学图像数据（含图像和分割标签）。
# 将3D数据沿z轴切分成2D切片，并保存为独立的.npz文件（若启用--split参数）。
# 生成数据集划分文件（train.txt/val.txt），记录每个样本的路径、预测头索引和类别数。
...
#                            3d切2d   指定输出lists文件名    并行进程数  预处理数据的根目录路径
# python make_dataset_txt.py --split --name lists_Synapse --n_jobs 8 --nnunet /path/to/your/nnUNet_preprocessed